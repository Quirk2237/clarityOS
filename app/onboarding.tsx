...
type OnboardingGoal = Database['public']['Tables']['onboarding_responses']['Row']['goal'];
type BusinessStage = Database['public']['Tables']['onboarding_responses']['Row']['business_stage'];
...
const GOAL_OPTIONS = [
  ...
  { value: 'build_confidence' as const, label: 'Build confidence in what I\'m selling' },
  { value: 'explain_what_i_do' as const, label: 'Be able to explain what I do' },
...
export default function OnboardingScreen() {
  ...
  const handleNext = () => {
    if (currentStep === 1) {
      if (!data.goal) {
        Alert.alert('Please select an option', 'Please choose your goal before continuing.');
        return;
      }
      if (data.goal === 'other' && !data.goalOtherText.trim()) {
        Alert.alert('Please specify', 'Please specify your goal in the text field.');
        return;
      }
      setCurrentStep(2);
  ...
  const handleSubmit = async () => {
    if (!data.businessStage) {
      Alert.alert('Please select an option', 'Please choose your business stage before continuing.');
      return;
    }
    ...
    <View className="flex-1">
      ...
      >
        {GOAL_OPTIONS.map((option) => (
          <View key={option.value} className="flex-row items-center gap-3 p-4 border border-border rounded-lg">
      ...
      {data.goal === 'other' && (
        <View className="mt-4">
          <Input
            ...
            onChangeText={(text) => setData(prev => ({ ...prev, goalOtherText: text }))}
            className="text-base"
          />
        </View>
      )}
  ...
  const renderQuestion2 = () => (
    <View className="flex-1">
      <Text className="text-2xl font-bold mb-2">What stage is your business in?</Text>
      <Text className="text-muted-foreground mb-8">
  ...
  const renderNavigationButtons = () => (
    <View className="flex-row gap-4 mt-8">
      ...
      <Button
        ...
        disabled={isSubmitting}
      >
        <Text>{currentStep === totalSteps ? (isSubmitting ? 'Saving...' : 'Complete') : 'Continue'}</Text>
      </Button>
    </View>
  );
