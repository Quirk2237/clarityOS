...
async function testRateLimit(endpoint) {
  ...
  for (let i = 0; i < 15; i++) {
    promises.push(
      testEndpoint(endpoint, testCases.validRequest, `Rate limit test ${i + 1}`)
    );
  }
...
async function runSecurityTests() {
  ...
  await testEndpoint(testEndpoint_name, testCases.validRequest, 'Valid request');
  await testEndpoint(testEndpoint_name, testCases.tooManyMessages, 'Too many messages');
  await testEndpoint(testEndpoint_name, testCases.messageTooLong, 'Message too long');
  await testEndpoint(testEndpoint_name, testCases.invalidMessages, 'Invalid messages format');
  await testEndpoint(testEndpoint_name, testCases.missingUserId, 'Missing user ID');
